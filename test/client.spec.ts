...
describe("the golem-base client", () => {
  log.info("Delete all existing entities...");
  (async () => {
    await client.deleteEntitiesAndWaitForReceipt(await client.getAllEntityKeys())
  })()
  log.info("Deleted all existing entities");
  ...
  it("should be able to create multiple entities", async () => {
    ...
    // The entity expires at the block after its TTL became 0, so we need to add 1
    expiryBlock = Number(await client.getBlockNumber()) + 25 + 1

    expect(receipts[0][0].expirationBlock).to.eql(expiryBlock)

    // Save this key for later
    entityKey = receipts[0][0].entityKey
  })
